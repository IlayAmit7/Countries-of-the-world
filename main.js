...
(function () { 
    ...
    $(function () {
        ...
        $("#get-all-countries-button").keyup(function (event) {  
        ...
        $("#search-countries-input").keyup(function (event) { 
            let keycode = (event.keyCode ? event.keyCode : event.which);
            if (keycode === '13') {
    ...
    function addCountriesToUI(countries) {
        for (let country of countries) {
            const regionsOfCountries = `<td>${country.name.common} ${country.flag} </td>`;
            const countriesNumbers = `<td>${country.population}</td>`;
            let openTd3 = `<td>`;
            ...
            let td3 = openTd3 + "</td>";
            const tr = `<tr>${regionsOfCountries}${countriesNumbers}${td3}</tr>`;
    ...
    function addCountriesRegionToUI(countriesArray) {
        ...
        for (const [region, numOfCountries] of regionCountriesMap.entries()) {
            if (numOfCountries !== 0) {
                const regionsOfCountries = `<td>${region}</td>`;
                const countriesNumbers = `<td>${numOfCountries}</td>`;
                const tr = `<tr>${regionsOfCountries}${countriesNumbers}</tr>`;
